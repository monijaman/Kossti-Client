        ...
        <div className="flex items-center justify-between pt-1 md:pt-2 pb-1 md:pb-2 border-t border-gray-100">
          ...
          <div className="flex items-center gap-0.5">
            {[...Array(5)].map((_, i) => (
              <span
                key={i}
                className={`text-sm md:text-lg transition-transform group-hover:scale-110 ${product.average_rating && i < Math.round(product.average_rating)
        ...
        <div className="flex items-center gap-2 md:gap-3">
          {product.brand?.name && (
            <div className="flex items-center gap-1 md:gap-2 min-w-0">
              <div className="w-6 md:w-8 h-6 md:h-8 bg-gradient-to-br from-blue-400 to-purple-500 rounded-md flex items-center justify-center shadow-md flex-shrink-0">
                <span className="text-xs font-bold text-white">
                  {displayBrandName.substring(0, 2).toUpperCase()}
                </span>
              </div>
              <span className="text-xs md:text-sm font-semibold text-gray-700 group-hover:text-blue-600 transition-colors truncate">
                {displayBrandName}
              </span>
            </div>
          )}
